// LegalTech — Renovación Urbana (Ley 29415) — SPA estática
...
async function fetchJson(path){
  // Cache-buster para GitHub Pages/CDN (evita ver datasets antiguos tras desplegar)
  const url = `${path}?v=${Date.now().toString(16)}`;
...
function renderNav(){
  ...
  nav.innerHTML = `
    <nav class="nav">
      ...
      <div class="picker">
        <div class="muted" style="font-size:12px;margin-bottom:6px">Proyecto activo</div>
        <select id="projectPicker">
          <option value="">— Sin proyecto activo —</option>
          ${(state.projects||[]).map(p=>`<option value="${esc(p.id)}" ${p.id===activeId?'selected':''}>${esc(p.nombre||p.id)}</option>`).join('')}
        </select>
      </div>
  ...
  const picker = nav.querySelector('#projectPicker');
  picker?.addEventListener('change', ()=>{
    state.activeProjectId = picker.value || '';
    saveState();
    // refresca vista actual para reflejar el proyecto activo
    render();
  });
}
...
function renderProjectList(host){
          ...
          return `
            <tr>
              ...
              <td>
                ...
                <button class="btn btn--danger" data-act="del" data-id="${esc(p.id)}">Eliminar</button>
              </td>
            </tr>
          `;
        }).join('')}
  ...
  host.addEventListener('click', (e)=>{
    const bSet = e.target.closest('button[data-act="set"]');
    const bDel = e.target.closest('button[data-act="del"]');

    if(bSet){
      state.activeProjectId = bSet.dataset.id;
      saveProjects();
...
Esto borra su checklist y datos locales (no se puede deshacer).`);
      if(!ok) return;
      ...
      saveProjects();
      toast('Proyecto eliminado', p.nombre);
      go();
      return;
    }
  });
...
function viewLibrary(params){
  ...
  el.innerHTML = `
    ...
    <div class="row" style="margin-top:12px">
      ...
      <div class="card">
        <div class="hd"><b>Pasos / Requisitos</b><div class="muted" style="font-size:12px;margin-top:4px">Vista operativa para gestión diaria (etapas → requisitos).</div></div>
        <div class="bd">
          <div class="muted" style="font-size:12px;margin-bottom:6px">Etapa</div>
          <select id="libStage"></select>
          <div id="libReqList" style="margin-top:10px"></div>
        </div>
      </div>
    </div>
      <div class="card">
  ...
  // Vista operativa: etapas → requisitos (para correlacionar norma con gestión diaria)
  const libStageSel = el.querySelector('#libStage');
  const libReqHost = el.querySelector('#libReqList');
  if(libStageSel && libReqHost){
    const templates = (state.rules && state.rules.stage_templates) ? state.rules.stage_templates : [];
    if(!templates.length){
      libStageSel.innerHTML = `<option value="">(Sin etapas configuradas)</option>`;
      libReqHost.innerHTML = `<div class="notice warn">Configura <b>stage_templates</b> en <code>data/rules.sample.json</code> para ver pasos y requisitos.</div>`;
    } else {
      libStageSel.innerHTML = templates.map((t,i)=>`<option value="${i}">${esc(t.nombre||('Etapa '+(i+1)))}</option>`).join('');
      const reqMap = requirementMap();
      const renderStage = ()=>{
        const i = parseInt(libStageSel.value||'0',10);
        const t = templates[Number.isFinite(i)?i:0];
        if(!t){ libReqHost.innerHTML = `<div class="notice">Selecciona una etapa.</div>`; return; }
        const ids = (t.requirement_ids||[]);
        if(!ids.length){ libReqHost.innerHTML = `<div class="notice warn">Esta etapa no tiene requisitos asociados en el dataset.</div>`; return; }
        const rows = ids.map(rid=>{
          const r = reqMap.get(rid);
          const name = r?.nombre || rid;
          const sec = r?.base_legal_ref ? byId.get(r.base_legal_ref) : null;
          const link = r?.base_legal_ref ? `#/library?sid=${encodeURIComponent(r.base_legal_ref)}&rid=${encodeURIComponent(rid)}` : '';
          return `
            <div class="item">
              <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                <span class="badge">${esc(rid)}</span>
                <b>${esc(name)}</b>
                ${r?.obligatorio?'<span class="badge warn">Obligatorio</span>':'<span class="badge">Opcional</span>'}
              </div>
              <div class="muted" style="margin-top:4px">${esc(r?.descripcion || '')}</div>
              <div style="margin-top:6px;display:flex;gap:10px;flex-wrap:wrap">
                ${sec?`<span class="badge">Base: ${esc(labelFor(sec))}</span>`:''}
                ${link?`<a class="link" href="${link}">Ver base legal</a>`:''}
              </div>
            </div>
          `;
        }).join('');
        libReqHost.innerHTML = `<div class="stack">${rows}</div>`;
      };
      libStageSel.addEventListener('change', renderStage);
      renderStage();
    }
  }
...
function viewWizard(){
  ...
  el.querySelector('#mk').addEventListener('click', ()=>{
    ...
    // Captura de datos urbanos clave desde el wizard
    p.metrado_m2 = Number(answers.Q6 || 0);
    p.familias = Number(answers.Q9 || 0);
    p.viviendas_interiores = Number(answers.Q10 || 0);
...
function viewProject(params){
    ...
    <div class="card" style="margin-top:12px">
      ...
      <div class="bd">
        ...
        <div class="row" style="margin-top:10px">
          ...
          <div>
            <div class="muted" style="font-size:12px;margin-bottom:6px">Familias / hogares (estimado)</div>
            <input id="familias" type="number" min="0" step="1" value="${esc(p.familias ?? 0)}">
          </div>
          <div>
            <div class="muted" style="font-size:12px;margin-bottom:6px">Viviendas interiores (estimado)</div>
            <input id="viviendas" type="number" min="0" step="1" value="${esc(p.viviendas_interiores ?? 0)}">
          </div>
          <div>
      ...
      <div class="bd" style="padding-top:0;display:flex;gap:10px;flex-wrap:wrap">
        ...
        <button class="btn btn--danger" id="deleteProject">Eliminar</button>
  ...
  el.querySelector('#save').addEventListener('click', ()=>{
    ...
    p.familias = Number(el.querySelector('#familias').value || 0);
    p.viviendas_interiores = Number(el.querySelector('#viviendas').value || 0);
    p.fechas = p.fechas || {};
  ...
  el.querySelector('#deleteProject')?.addEventListener('click', ()=>{
    const ok = confirm(`Eliminar el proyecto "${p.nombre}"?\n\nEsto borra su checklist y datos locales (no se puede deshacer).`);
    if(!ok) return;
...
function viewChecklist(params){
  ...
  function render(){
    ...
    body.innerHTML = `
      <div class="notice"><b>Estado etapa:</b> ${esc(st.estado||'—')} • Progreso: <b>${pr.pct}%</b> (OK:${pr.ok}, Parcial:${pr.partial}, Total:${pr.denom})</div>

      <div class="card" style="margin-top:10px">
        <div class="hd"><b>Control de requisitos</b><div class="muted" style="font-size:12px;margin-top:4px">Usa el desplegable para ubicar un requisito y, si corresponde, descartarlo (No aplica).</div></div>
        <div class="bd">
          <div class="row">
            <div>
              <div class="muted" style="font-size:12px;margin-bottom:6px">Etapa</div>
              <select id="stageJump">
                ${(p.etapas||[]).map((s,i)=>`<option value="${i}" ${i===idx?'selected':''}>${esc(s.nombre)}</option>`).join('')}
              </select>
              <div class="muted" style="font-size:12px;margin:10px 0 6px">Ir a requisito</div>
              <select id="reqJump">
                <option value="">— Selecciona —</option>
                ${(st.checklist_items||[]).map(ci=>{
                  const r = reqMap.get(ci.requirement_id);
                  const label = (r?.nombre || ci.requirement_id);
                  return `<option value="${esc(ci.requirement_id)}">${esc(label)} • [${esc(ci.estado)}]</option>`;
                }).join('')}
              </select>
            </div>
            <div>
              <div class="muted" style="font-size:12px;margin-bottom:6px">Acción</div>
              <div style="display:flex;gap:8px;flex-wrap:wrap">
                <button class="btn btn--ghost" id="btnGoReq">Ir</button>
                <button class="btn btn--danger" id="btnDiscard">Descartar (No aplica)</button>
                <button class="btn btn--ghost" id="btnRestore">Restaurar (Pendiente)</button>
              </div>
            </div>
            <div>
              <div class="muted" style="font-size:12px;margin-bottom:6px">Vista</div>
              <label class="muted" style="display:flex;gap:8px;align-items:center"><input type="checkbox" id="hideNA"> Ocultar descartados</label>
            </div>
          </div>
        </div>
      </div>
    ...
    // control panel
    const sel = body.querySelector('#reqJump');
    const stageSel = body.querySelector('#stageJump');
    stageSel?.addEventListener('change', ()=>{
      const ni = parseInt(stageSel.value||'0',10);
      idx = Number.isFinite(ni) ? ni : 0;
      render();
    });
    const hideNA = body.querySelector('#hideNA');
    const btnGo = body.querySelector('#btnGoReq');
    const btnDisc = body.querySelector('#btnDiscard');
    const btnRes = body.querySelector('#btnRestore');

    const applyHide = ()=>{
      const hide = hideNA?.checked;
      body.querySelectorAll('[data-rid]').forEach(card=>{
        const rid = card.dataset.rid;
        const ci = st.checklist_items.find(x=>x.requirement_id===rid);
        if(!ci) return;
        card.style.display = (hide && ci.estado==='no_aplica') ? 'none' : '';
      });
    };

    hideNA?.addEventListener('change', applyHide);
    applyHide();

    btnGo?.addEventListener('click', ()=>{
      const rid = sel?.value;
      if(!rid) return toast('Selecciona un requisito', 'Usa el desplegable.');
      const card = body.querySelector(`[data-rid="${CSS.escape(rid)}"]`);
      if(!card) return toast('No se encontró', 'Ese requisito no está en la etapa actual.');
      // Asegura que el <details> esté abierto y se vea con claridad (header fijo)
    ...
    btnDisc?.addEventListener('click', ()=>{
      const rid = sel?.value;
      if(!rid) return toast('Selecciona un requisito', 'Usa el desplegable.');
      setStatus(rid, 'no_aplica');
    });

    btnRes?.addEventListener('click', ()=>{
      const rid = sel?.value;
      if(!rid) return toast('Selecciona un requisito', 'Usa el desplegable.');
      setStatus(rid, 'pendiente');
    });
...
function renderReq(ci, reqMap){
  ...
  const fields = (r.entradas||[]).map(f=>{
    ...
    if(f.type==='select'){
      const opts = (f.options||[]).map(o=>{
        const val = String(o);
        return `<option value="${esc(val)}" ${String(v??'')===val?'selected':''}>${esc(val)}</option>`;
      }).join('');
      return `<div>
        <div class="muted" style="font-size:12px;margin-bottom:6px">${esc(f.label)}${f.required?' *':''}</div>
        <select data-k="${esc(f.key)}">
          <option value="">—</option>
          ${opts}
        </select>
      </div>`;
    ...
    if(f.type==='boolean'){
      return `<div>
        <div class="muted" style="font-size:12px;margin-bottom:6px">${esc(f.label)}${f.required?' *':''}</div>
        <input type="checkbox" data-k="${esc(f.key)}" ${v===true?'checked':''}>
      </div>`;
    }
    ...
    if(f.type==='date'){
      return `<div>
        <div class="muted" style="font-size:12px;margin-bottom:6px">${esc(f.label)}${f.required?' *':''}</div>
        <input type="date" data-k="${esc(f.key)}" value="${esc(v||'')}">
      </div>`;
    }
    ...
    if(f.type==='number'){
      return `<div>
        <div class="muted" style="font-size:12px;margin-bottom:6px">${esc(f.label)}${f.required?' *':''}</div>
        <input type="number" data-k="${esc(f.key)}" value="${esc(v??'')}">
      </div>`;
    }

    return `<div>
      <div class="muted" style="font-size:12px;margin-bottom:6px">${esc(f.label)}${f.required?' *':''}</div>
      <input type="text" data-k="${esc(f.key)}" value="${esc(v??'')}">
    </div>`;
  }).join('');
  ...
  const statusBadge = (()=>{
    const s = ci.estado || 'pendiente';
    const cls = s==='cumplido' ? 'ok' : (s==='parcial' ? 'warn' : (s==='no_aplica' ? 'muted' : 'bad'));
    const label = s==='cumplido' ? 'Cumplido' : (s==='parcial' ? 'Parcial' : (s==='no_aplica' ? 'No aplica' : 'Pendiente'));
    return `<span class="badge ${cls}">${label}</span>`;
  })();

  return `
    <details class="card" style="margin-bottom:12px" data-rid="${esc(ci.requirement_id)}" open>
      <summary class="hd" style="cursor:pointer;list-style:none">
        <div style="display:flex;gap:10px;justify-content:space-between;flex-wrap:wrap;align-items:flex-start">
          <div>
            <b>${esc(r.nombre||r.id)}</b> ${statusBadge}
      ...
      <div class="bd">
        ...
        <div class="row">
          <div>
            <div class="muted" style="font-size:12px;margin-bottom:6px">Evidencia esperada</div>
...
function buildExpedienteHTML(p){
  ...
  const cover = `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      ...
      ${row('Familias / hogares', String(p.familias ?? ''))}
      ${row('Viviendas interiores', String(p.viviendas_interiores ?? ''))}
